/*
 * TODO: should be able to simplify this further, and integrate into the 'lcd.c' and 'lcd.h' files instead: do we want to move from CPP class to plain C structures as well?
 * 
...
void ERM19264_UC1609_T::LCDChar(unsigned char character)
{
  ...
  {
    send_data((pgm_read_byte(MiyoCalcFont + (((character-'A'+11)*12)) + column)));
  }
  UC1609_FONTPADDING;
  UC1609_CS_SetHigh;
}
...
{
  //send_command(UC1609_GN_PM, 0);
  //send_command(UC1609_GN_PM, cont);
  //send_command(UC1609_GN_PM, (cont & 0x3F) << 1);
  UC1609_CD_SetLow;
  UC1609_CS_SetLow;
